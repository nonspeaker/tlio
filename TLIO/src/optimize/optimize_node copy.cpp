...
int main(int argc, char **argv) {
    ...
    ros::Publisher pubCorrectOdom = nh.advertise<nav_msgs::Odometry>("/odometry_correct", 100000);
    ...
    keyframe_file.open("/home/tuyanchen/Livox2/TLIO/src/TLIO/evaluate/keyframes.csv", std::ios::out);

    if (keyframe_file.is_open()) {
        keyframe_file << "%time,field.header.seq,field.header.stamp,"
    ...
    while (ros::ok()) {
        ...
        if(optimizer.isKeyFrame(current_pose)) {
            ...
            odom.header.stamp = ros::Time(latestPose.time);
            odom.header.frame_id = "camera_init";
            odom.child_frame_id = "camera_init";
            odom.pose.pose.position.x = latestPose.x;
            odom.pose.pose.position.y = latestPose.y;
            odom.pose.pose.position.z = latestPose.z;
            Eigen::Quaterniond q = Eigen::AngleAxisd(latestPose.yaw, Eigen::Vector3d::UnitZ()) *
                                    Eigen::AngleAxisd(latestPose.pitch, Eigen::Vector3d::UnitY()) *
                                    Eigen::AngleAxisd(latestPose.roll, Eigen::Vector3d::UnitX());
            ...
            keyframe_file << std::scientific << std::setprecision(14)
                      << odom.header.stamp.toNSec() << "," // 时间戳（纳秒）
                      << odom.header.seq << ","           // 序列号
                      << odom.header.stamp.toNSec() << ","; // 时间戳（纳秒）

            keyframe_file << std::setprecision(15)
                      << odom.pose.pose.position.x << ","
                      << odom.pose.pose.position.y << ","
                      << odom.pose.pose.position.z << "," // 位置
                      << odom.pose.pose.orientation.x << ","
                      << odom.pose.pose.orientation.y << ","
                      << odom.pose.pose.orientation.z << ","
                      << odom.pose.pose.orientation.w << "\n"; // 旋转（四元数）

            keyframe_file.flush(); // 刷新缓冲区，确保数据写入磁盘
